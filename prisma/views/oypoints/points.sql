WITH `potential_grade_allocated_results` AS (
  SELECT
    ...
    `oypoints`.`result`.`scorer_id` AS `scorer_id`,
...
`grade_allocated_result` AS (
  SELECT
    ...
    `ranking`.`scorer_id` AS `scorer_id`,
  ...
  FROM
    (
      SELECT
        ...
        `potential_grade_allocated_results`.`scorer_id` AS `scorer_id`,
...
`grade_winning_time` AS (
  SELECT
    ...
    `grade_allocated_result`.`scorer_id` AS `scorer_id`,
(
      SELECT
  ...
  GROUP BY
    ...
    `grade_allocated_result`.`scorer_id`
  ORDER BY
    `grade_allocated_result`.`league_id`,
    `grade_allocated_result`.`season_id`,
    `grade_allocated_result`.`event_number`
...
(
      CASE
        WHEN (
          ...
          SELECT
            `oypoints`.`scorer`.`scale`
          FROM
            `oypoints`.`scorer`
          WHERE
            (
              `oypoints`.`scorer`.`scorer_id` = `grade_allocated_result`.`scorer_id`
            )
        )
        WHEN (
          (`grade_winning_time`.`winning_time` IS NULL)
        ...
        WHEN (
          ...
          SELECT
            `oypoints`.`scorer`.`penalty_points`
          FROM
            `oypoints`.`scorer`
          WHERE
            (
              `oypoints`.`scorer`.`scorer_id` = `oypoints`.`result_collapsed`.`scorer_id`
            )
        )
        WHEN (
      ...
      (
        ...
        LEFT JOIN `grade_winning_time` ON(
          (
            ...
            AND (
              `grade_winning_time`.`scorer_id` = `grade_allocated_result`.`scorer_id`
            )
          )
        )
      )
